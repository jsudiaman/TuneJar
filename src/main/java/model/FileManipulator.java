...
public final class FileManipulator {
	...
	public static Set<File> initialSetup(Stage stage) {
		...
		if (chosenDirectory == null) {
			LOGGER.log(Level.WARNING,
					"User pressed 'cancel' when asked to choose a directory.");
		} else {
	 ...
	 *
	 * @param file
	 *            The file to read directories from
	 ...
	 *
	 * @param fileName
	 *            Where to write the output. The file will be overwritten if it
	 *            exists.
	 * @param fileSet
	 *            A set of files
	 * @throws IOException
	 ...
	 * be wrapped up in an ordered list.
	 *
	 * @param directory
	 *            A File object that is a directory.
	 * @return A list containing all the Song objects, or null if the File
	...
	public static List<Song> songList(@NotNull File directory) {
		...
		for (File f : directory.listFiles()) {
			// If a directory was found, add the mp3 files in that directory as
			// well.
			if (f.isDirectory()) {
				list.addAll(songList(f));
			} else {
				// Attempt to construct a song object. If successful, add it to
				// the list.
				...
				try {
					...
					LOGGER.log(
							Level.SEVERE,
							"Failed to construct a song object from file: "
									+ f.toString(), e);
				}
			}
		}
	 ...
	 * out of each one. All of the created playlists are then wrapped in a list
	 * and returned.
	 *
	 * @param directory
	 *            The directory in which playlists should be searched for
	...
	public static List<Playlist> allPlaylists(File directory) {
		//Initiliaze the playlist list or return null
		if (directory == null || !directory.isDirectory())
			return null;
		List<Playlist> list = new ArrayList<>();
		//iterate through each file in the directory
		for (File f : directory.listFiles()) {
			//if the directory was found add all play lists from the directory
			if (f.isDirectory()) {
				list.addAll(allPlaylists(f));
			} else {
				//attempt to construct a  play list object and add it to the list if successful 
				if (!f.toString().endsWith(".m3u"))
					continue;
				try {
					Playlist playlist = new Playlist();
					playlist.loadInM3U(f);
					list.add(playlist);
				} catch (UnsupportedTagException | InvalidDataException
						| IOException e) {
					LOGGER.log(Level.SEVERE,
							"Failed to construct a play list object from file: "
									+ f.toString(), e);
				}
