...
public abstract class Song {
   ...
   * <p>
   * <p>
   * Note that this constructor does not initialize <code>audioFile</code>. To avoid possible NPEs,
   * care should be taken to ensure that at the very least, <code>audioFile</code> is set to a
   * meaningful value <b>immediately</b> after this constructor is called.
   * </p>
   */
  protected Song() {
  ...
  /**
   * Checks if the song can be edited. Please note this implies that <b>all</b> fields are editable
   * - Title, Artist, and Album. It also implies that changes can be made to the audio file itself
   * (not just this object). If such an operation is unsupported, it is acceptable to hard-code this
   * method to return <code>false</code>. In that case, setters should also throw an
   * {@link AssertionError}, since setters should <b>not</b> be accessible if this method returns
   * false.
   *
